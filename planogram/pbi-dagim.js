/**
 * Fetch דגים (chilled fish) live data from Power BI / Fabric (replaces דגים.xlsx)
 * Returns: Map<makat, { desc, stock, daySales, pakuot, daySalesAll, pakuotAll, fam, nameEn }>
 *
 * Families (source: "משפחת מוצר לפי מחסן.xlsx", sheet דגים):
 *   דגים NORD PORT    → anchors 403001, 1000, 1122
 *   דגים SANTA BREMOR → anchors 1006, 1175
 *
...
// Known makatim from each dagim family — used to discover family codes at runtime via MLAY
const DAGIM_ANCHORS = ['403001', '1000', '1122', '1006', '1175'];
...
async function fetchDagimFromBI() {
  ...
  const anchorSet = DAGIM_ANCHORS.map(m => `"${m}"`).join(',');
  ...
  console.log(`Power BI דגים: ${total} total | ${withStock} with stock | ${withSales} with sales`);
  return { dagimData: result, nameEnMap };
}

module.exports = { fetchDagimFromBI };
