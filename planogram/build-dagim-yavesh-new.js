...
(async () => {
  // ── Step 1: Read layout + working picks from existing dagim-yavesh-base.json ─
  ...
  const bdPicks = loadBreiraDefault('דג יבש');
  const picks = {};
  const bdMakatSet = new Set();
  for (const [bay, bd] of Object.entries(bdPicks)) {
    const data = makatDataMap[bd.makat];
    picks[bay] = { makat: bd.makat, fam: data?.fam || bd.fam || null, name: data?.name || bd.name || null, printOrder: bd.printOrder ?? null, haluka: bd.haluka ?? null };
