# mahsan-planogram — Archive (сессии 2026-05-12 – 2026-07-07)

Старые сессии MAHSAN PLANOGRAM, вынесены из [[mahsan-planogram]] для читаемости основного файла. Актуальный статус, архитектура и последние сессии — в основном файле.
...
- **Страница תוקף** (`📦 תוקף` кнопка в тулбаре):
  ...
  - Тотал מלаי сверху: один значок `סה"כ מלאי: N קרט` (аshd+zafn)
...
**Страница הזמנה — открытые заказы и מלаי всех складов:**
...
- **Логика**: `[הזמנות רכש פתוחות PLUS מלאי זמין]` = מלаי всех складов + заказы поставщику. `openOrders` = чисто заказы (мера − все склады). `daysStk` = сколько дней хватит (все склады + заказы в пути).
...
- Sidebar убран полностью; фильтры מלаי אשדוד/MMD перенесены в горизонтальные строки `#skrow / .skline`
- `col-g` (#e8f0f8) — серая заливка на 5 колонках: מלаי MMD, מכר ממוצע, המלצה, הזמנה ידנית, תוקף הזמנה
...
**הזמנה דגים — цвета, פיזור, Excel מיקום:**
...
**Open (не закрыто на 2026-07-07)**: Excel סטנד — «V EXEL סטנד TAKI NA POPADAYUT», ~908 SKU с `haluka: null` сортируются в конец Excel (haluka→printOrder).
